...
#-----------------------------------------------------------
   if {"Cyclone 10 LP" == "MAX 10" || "Cyclone 10 LP" == "Max 10"} {
...
set_max_delay -from [get_registers *] -to ${HB_csn_ports}   [expr (0.6* 6.666666666666667)]
set_max_delay -from [get_registers *] -to ${HB_clk_ports}   [expr (0.6* 6.666666666666667)]
...
#-----------------------------------------------------------
create_clock -name virt_rwds_clk -period 6.666666666666667

#derive clocks and clock uncertainty
derive_clock_uncertainty

#get rwds in port names
set rwds_in_ports [get_ports -no_case -nowarn HB*rwds]
...
for {set hb_cs 0} {$hb_cs < $rwds_num_pins} {incr hb_cs} {
   ...
   #-----------------------------------------------------------
   create_clock -name $rwds_clk_name  -period 6.666666666666667 ${HB_rwds_in_ports}
